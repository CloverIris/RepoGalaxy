...
public sealed class OAuthCodeFlowService
{
    ...
    public async Task<TokenResponse?> ExecuteOAuthFlowAsync(TimeSpan? timeout = null, CancellationToken cancellationToken = default)
    {
        ...
        var result = await callback.WaitForCallbackAsync(timeout ?? TimeSpan.FromMinutes(5), cancellationToken);
        if (result == null) return null;
        if (!string.IsNullOrEmpty(result.Error)) throw new InvalidOperationException("GitHub 授权已取消或失败。");
        if (string.IsNullOrEmpty(result.Code) || !CryptographicOperations.FixedTimeEquals(System.Text.Encoding.UTF8.GetBytes(state), System.Text.Encoding.UTF8.GetBytes(result.State ?? string.Empty)))
